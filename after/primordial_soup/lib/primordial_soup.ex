defmodule PrimordialSoup do
  ...
  @default_options [
    ...
    progress_function: &PrimordialSoup.show_progress/1,
    record_genealogy: false,
    record_statistics: false,
    reinsertion_type: :pure,
    regenerate_probability: 0.5,
    regenerate_choices: 0..1,
    select_dupes: false,
    selection_rate: 0.8,
  ...
  def cross(parents, options) do
    ...
    |> Enum.flat_map(fn [p1, p2] ->
      [c1, c2] = apply(Crossover, crossover_type, [p1, p2, options])
  ...
  def mutate(population, options) do
    ...
    |> Enum.map(fn chromosome ->
      mutant = apply(Mutation, mutation_type, [chromosome, options])
  ...
  defp terminate(scored_populations, terminate?, options) do
    ...
    progress_function = Keyword.fetch!(options, :progress_function)

    scored_populations
    |> Enum.find(fn {[best | _rest], _generation} = {population, generation} ->
      if show_progress do
        progress_function.(best)
      end
